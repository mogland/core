...
export class ImageService {
  ...
  getImagesFromMarkdown(text: string) {
    ...
    ast.forEach((node: any) => { // 遍历 AST
      if (node.type === 'image') { // 如果是图片
        images.push(node.href) // 将图片 URL 添加到数组
        return // 结束遍历
      }
      if (node.tokens && Array.isArray(node.tokens)) { // 如果是子节点
        node.tokens.forEach(token => {  // 遍历子节点
          if (token.type === 'image') {
  ...
  async recordImageMeta<T extends WriteBaseModel>(
    ...
    for (const src of images) {
      ...
      if (origin && origin.src === src && ['height', 'width', 'type', 'accent'].every((key) => keys.has(key) && origin[key])) { // shit code
        result.push(origin)
        continue
      }
      ...
      const promise = new Promise<ImageModel>((resolve) => {
        ...
        this.getOnlineImageMeta(src).then(({ size, color }) => {
          const fileName = src.split('/').pop() // 获取图片名称
          this.logger.debug(`[${fileName}]: height: ${size.height}, width: ${size.width}, color: ${color}`,)
    ...
    await model.findByIdAndUpdate( // 更新文档中的 images
      id,
      {
