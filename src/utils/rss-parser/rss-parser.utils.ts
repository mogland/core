/*
 * @FilePath: /nx-core/src/utils/rss-parser/rss-parser.utils.ts
 * @author: Wibus
 * @Date: 2022-07-11 16:41:00
 * @LastEditors: Wibus
 * @LastEditTime: 2022-07-11 17:31:49
 * Coding With IU
 */
import utils from './rss-parser.tools';
import xml2js from 'xml2js';
import url from 'url';
import fields from './rss-parser.fields';
...
class Parser {
  ...
  parseString(xml: string, callback?: undefined) {
  ...
  parseURL(feedUrl, callback, redirectCount=0) {
    let xml = '';
    const get = this.http.axiosRef;
    const urlParts = url.parse(feedUrl);
    const headers = Object.assign({}, DEFAULT_HEADERS, this.options.headers);
    let timeout;
    let prom = new Promise((resolve, reject) => {
      const requestOpts = Object.assign({headers}, urlParts, this.options.requestOptions);
      const req = get(requestOpts, (res) => {
