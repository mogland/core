...
export class MarkdownController {
  ...
  async exportArticleToMarkdown(@Res() res,@Query() query: ExportMarkdownQueryDto) {
    ...
    const buffer = await rtzip.generateAsync({
      ...
      comment: 'Generated by NEXT-SERVER',
    })
  ...
  @Get('/export/html')
  @HTTPDecorators.Bypass
  @ApiProperty({ description: '使用模板编译静态页面' })
  async exportArticleToHTML(@Res() res) {
    // const { show_title: showTitle, slug, yaml } = query
    const allArticles = await this.service.extractAllArticle()
    const { posts } = allArticles
    ...
    const convertor = async <
      ...
      // }
      // console.log(item.content)
      return await this.service.htmlBuilder(item, type, extraData)

    }


    const convertPost = posts.map(async (post) =>
      await convertor(post!,'/posts', {
        categories: post.slug,
        type: '/post',
        path: `${post.path}`,
      }),
    )
    console.log(await convertPost)
    return await res.send(convertPost)
