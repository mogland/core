...
export class CategoryController {
  ...
  async getCategoryByCategoryIdOrTag(
    ...
    const data = isValidObjectId(query)  // 判断是否是ObjectId
      ? await this.categoryService.model.findById(query).sort({ created: -1 }).lean()  
      : await this.categoryService.model.findOne({ slug: query }).sort({ created: -1 }).lean()

    if (!data) {
      throw new CannotFindException()
    }

    const children = await this.categoryService.findCategoryPost(data._id, {
  ...
  async update(@Param() { id }: MongoIdDto, @Body() { type, slug, name }: CategoryModel) {
    await this.categoryService.model.updateOne({ _id: id }, { type, slug, name }) // 更新分类信息
  ...
  async deleteCategory(@Param() { id }: MongoIdDto): Promise<any> {
    ...
    const postsInCategory = await this.categoryService.findPostsInCategory(category.id)
    if (postsInCategory.length > 0) {
      throw new BadRequestException('该分类下有文章，不能删除')
    }
    const res = await this.categoryService.model.deleteOne({ _id: category.id }) // 删除分类
