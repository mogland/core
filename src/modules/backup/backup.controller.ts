...
import { BadRequestException, Controller, Get, Header, Post, Req } from '@nestjs/common';
...
export class BackupController {
  ...
  @Header(
    ...
    `attachment; filename="nx-space-backup-${getMediumDateTime(new Date())}.tar.gz"`,  // filename 参数用来告诉浏览器下载的文件名。 attachment 参数用来告诉浏览器，文件是附件，而不是直接显示。
  )
  @Bypass
  ...
  private async ValidMultipartFields(req: FastifyRequest): Promise<MultipartFile> {
